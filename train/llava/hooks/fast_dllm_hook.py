...
class FastDLLMGenerationHook:
    ...
    def _find_mask_spans(self, mask_ids):
        spans = []
        in_run, s = False, -1
        for i, v in enumerate(mask_ids.tolist()):
            if v and not in_run:
                in_run, s = True, i
            elif not v and in_run:
                if i > s + 1:
                    spans.append((s, i))  # [s, i)
                in_run = False
        if in_run:
            spans.append((s, len(mask_ids)))
        return spans

    def _freeze_tail(self, x_tokens, x_embeds, start_pos, end_pos, pad_id, pad_embed):
        x_tokens[:, start_pos:end_pos] = pad_id
        x_embeds[:, start_pos:end_pos, :] = pad_embed.expand(1, end_pos - start_pos, -1)
        return x_tokens, x_embeds

    def _count_mask_in_span(self, x_embeds, masked_embed, s, e):
        mask_slice = torch.all(torch.abs(x_embeds[:, s:e, :] - masked_embed) < 1e-5, dim=2)
        return int(mask_slice.sum().item()), mask_slice
    ...
    def _fast_generate_with_embeds(
        ...
        **kwargs
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            # if resp_template is not None:
            #
            #     dynamic_decoding_config = kwargs['dynamic_decoding_config']
            #     mdm_start_id = dynamic_decoding_config['mdm_start_id']
            #     mdm_end_id = dynamic_decoding_config['mdm_end_id']
            #     pad_id = dynamic_decoding_config['pad_id']
            #     min_field_len = dynamic_decoding_config['min_field_len']
            #     end_prob_threshold = dynamic_decoding_config['end_prob_threshold']
            #     remain_ratio_gate = dynamic_decoding_config['remain_ratio_gate']
            #     calibrate_end_logits = dynamic_decoding_config['calibrate_end_logits']
            #
            #
            #     pad_embed = self.model.model.embed_tokens(torch.tensor([pad_id], device=inputs_embeds.device)).float()
            ...
            if resp_template is not None:
                ...
                resp_token_ids = tokenizer.encode(resp_template, add_special_tokens=False)
                resp_token_ids = torch.tensor(resp_token_ids, dtype=torch.long, device=inputs_embeds.device).unsqueeze(0)
                resp_embeds = self.model.model.embed_tokens(resp_token_ids)  # (1, s, d)
                gen_length = max(resp_embeds.shape[1], gen_length)

                resp_mask_ids = (resp_token_ids == mask_id)
            ...
            if resp_template is not None:
                ...
                s = resp_embeds.shape[1]
                gen_slice = slice(gen_start, gen_start + s)
                cur = x_embeds[:, gen_slice, :]  # (1, s, d)

                x_embeds[:, gen_slice, :] = torch.where(
                    resp_mask_ids.unsqueeze(-1),  # (1, s, 1)
                    masked_embed.expand(1, s, -1),
                    resp_embeds
                )

                # field_spans = []
                # rel_spans = self._find_mask_spans(resp_mask_ids[0])
                # for rs, re in rel_spans:
                #     abs_s = gen_start + rs
                #     abs_e = gen_start + re
                #     field_spans.append({'s': abs_s, 'e': abs_e, 'ended': False})
            ...
            if resp_token_ids is not None:
                gen_slice = slice(inputs_embeds.shape[1], inputs_embeds.shape[1] + resp_token_ids.shape[1])
                x[:, gen_slice] = resp_token_ids
            ...
            for num_block in range(num_blocks):
                ...
                while True:
                    ...
                    if i % prefix_refresh_interval == 0:
                        ...
                        # if resp_template is not None:
                        #
                        #     pos_gate = torch.zeros((x.shape[1],), dtype=torch.bool, device=x.device)
                        #     for span in field_spans:
                        #         if span['ended']:
                        #             continue
                        #         s, e = span['s'], span['e']
                        #         gate_end = min(s + min_field_len, e)
                        #         if gate_end > s:
                        #             pos_gate[s:gate_end] = True
                        #
                        #     if calibrate_end_logits != 0.0:
                        #         logits[:, :, mdm_end_id] += calibrate_end_logits
                        #
                        #     end_logits = logits[:, :, mdm_end_id]
                        #     end_logits[:, pos_gate] = -float('inf')
                        #     logits[:, :, mdm_end_id] = end_logits
                        ...
                        # if resp_template is not None:
                        #     end_probs_full = torch.softmax(logits, dim=-1)[:, :, mdm_end_id]
                        #     for span in field_spans:
                        #         if span['ended']:
                        #             continue
                        #         s, e = span['s'], span['e']
                        #         num_mask, mask_slice = self._count_mask_in_span(x_embeds, masked_embed, s, e)
                        #         total_len = e - s
                        #         if num_mask == 0:
                        #             span['ended'] = True
                        #             continue
                        #
                        #     if num_mask / max(1, total_len) <= remain_ratio_gate:
                        #         span_end_probs = end_probs_full[:, s:e]  # (1, e-s)
                        #         mask_idx_1d = mask_slice[0]  # (e-s,)
                        #         if mask_idx_1d.any():
                        #             pe = float(span_end_probs[0, mask_idx_1d].max().item())
                        #             if pe >= end_prob_threshold:
                        #                 span_tokens = x[0, s:e]
                        #                 pos = (span_tokens == mdm_end_id).nonzero(as_tuple=True)[0]
                        #                 if len(pos) > 0:
                        #                     end_pos = s + pos[0].item()
                        #                 else:
                        #                     first_mask_rel = mask_idx_1d.nonzero(as_tuple=True)[0][0].item()
                        #                     end_pos = s + first_mask_rel
                        #
                        #                 if end_pos >= s + min_field_len:
                        #                     x, x_embeds = self._freeze_tail(x, x_embeds, end_pos + 1, e, pad_id, pad_embed)
                        #                     span['ended'] = True

                    else:
                        # if resp_template is not None:
                        #     pos_gate = torch.zeros((x.shape[1] - block_start,), dtype=torch.bool, device=x.device)
                        #     for span in field_spans:
                        #         if span['ended']:
                        #             continue
                        #         s, e = span['s'], span['e']
                        #         gate_end = min(s + min_field_len, e)
                        #         gs = max(block_start, s)
                        #         ge = max(block_start, gate_end)
                        #         if ge > gs:
                        #             pos_gate[gs - block_start: ge - block_start] = True
                        #
                        #     if calibrate_end_logits != 0.0:
                        #         logits[:, :, mdm_end_id] += calibrate_end_logits
                        #
                        ...
                        # if resp_template is not None:
                        #     end_probs_block = torch.softmax(logits, dim=-1)[:, :, mdm_end_id]  # (1, L_block)
                        #
                        #     for span in field_spans:
                        #         if span['ended']:
                        #             continue
                        #         s, e = span['s'], span['e']
                        #         bs = max(s, block_start)
                        #         be = min(e, block_end)
                        #         if be <= bs:
                        #             continue
                        #
                        ...
                        #
                        #             print("Step: ", i)
                        #             print(num_mask, span, field_spans)
                        #
                        #         if num_mask / max(1, total_len) <= remain_ratio_gate:
                        #             local_start = bs - block_start
                        #             local_end = be - block_start
                        #
                        #             field_mask_1d = mask_slice[0]  # (e-s,)
                        #             block_mask_1d = field_mask_1d[bs - s: be - s]  # (be-bs,)
                        #             if block_mask_1d.any():
                        #                 span_end_probs_blk = end_probs_block[:, local_start:local_end]  # (1, be-bs)
                        #                 pe = float(span_end_probs_blk[0, block_mask_1d].max().item())
                        #
                        #                 if i % 155 == 0:
                        #                     print(block_mask_1d)
                        #                     print(pe, end_prob_threshold)
                        #                     k = 10  # top k
                        #                     probs = end_probs_block[0]  # (L_block,)
                        #                     topk_vals, topk_idx = torch.topk(probs, k)  # (k,), (k,)
                        #                     print(f"\n[DEBUG step {i}] top-{k} end probs in block:")
                        #                     for rank, (idx, val) in enumerate(zip(topk_idx.tolist(), topk_vals.tolist())):
                        #                         abs_pos = block_start + idx
                        #                         print(
                        #                             f"  #{rank + 1}: pos={abs_pos:4d} (block idx {idx:3d})  prob={val:.4f}  token={x[0, abs_pos].item()}")
                        #
                        #                 if pe >= end_prob_threshold:
                        #                     span_tokens = x[0, s:e]
                        #                     pos = (span_tokens == mdm_end_id).nonzero(as_tuple=True)[0]
                        ...
                        #                         end_pos = s + pos[0].item()
                        #                     else:
                        #                         first_mask_rel = field_mask_1d.nonzero(as_tuple=True)[0][0].item()
                        #                         end_pos = s + first_mask_rel
                        #
                        #                     if end_pos >= s + min_field_len:
                        #                         x, x_embeds = self._freeze_tail(x, x_embeds, end_pos + 1, e, pad_id, pad_embed)
