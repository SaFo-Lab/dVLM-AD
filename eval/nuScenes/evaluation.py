...
def planning_evaluation(pred_trajs_dict, config):
    ...
    if config.metric == "uniad":
        from nuScenes.metric_uniad import PlanningMetric
        ...
        from nuScenes.metric_stp3 import PlanningMetric
    ...
    with open("/weka/home/xliu316/scratchcxiao13/yingzi/workspace/nuscenes/nuscenes2d_ego_temporal_infos_val.pkl",
              'rb') as f:
        data = pickle.load(f)['infos']

    gt_trajs_dict = {item['token']: item['gt_planning'][:, :, :2] for item in data}
    gt_trajs_mask_dict = {item['token']: item['gt_planning_mask'][:, :, :2] for item in data}

    for index, token in enumerate(tqdm(gt_trajs_dict.keys())):
        if token not in pred_trajs_dict.keys():
            continue
    ...
    if config.metric == "uniad":
        method = (config.method, "{:.2f}".format(scores["L2"][1]), "{:.2f}".format(scores["L2"][3]),
                  ...
                  "{:.2f}".format(
                      100 * (scores["obj_box_col"][1] + scores["obj_box_col"][3] + scores["obj_box_col"][5]) / 3.))
        ...
        method = (config.method, "{:.2f}".format(results["plan_L2_1s"]), "{:.2f}".format(results["plan_L2_2s"]),
...
def load_pred_trajs_from_file(path, config):
    ...
    pred_trajs_dict = {line['sample_id'].split("_")[-1]: process(line['conversations'][-1]['value']) for line in
                       pred_data if line['sample_id'].split("_")[-1] in gt_trajs_dict.keys()}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method', type=str, help='name of the method being evaluated, used for table print',
                        default='Agent-Driver')
    parser.add_argument('--result_file', type=str, help='path to the result file',
                        default='temp_results/refined_trajs_dict_0.0_5.0_1.265_7.89.pkl')
    ...
    pred_trajs_dict = load_pred_trajs_from_file(result_file, config)
    planning_evaluation(pred_trajs_dict, config)


"""
python -m nuScenes.evaluation \
    --method dvlm-ad \
    --result_file /weka/home/xliu316/scratchcxiao13/yingzi/LLaDA-V/eval/nuScenes/ad_finetune_nusc_cot_planning_28k_ckpt1200.json \
    --metric stp3 \
    --gt_folder nuScenes/metrics
"""