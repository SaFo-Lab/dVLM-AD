...
def collect_leaf_strings(obj):
    ...
    if isinstance(obj, dict):
        for v in obj.values():
            vals.extend(collect_leaf_strings(v))
...
def build_supervised_mask_for_assistant(encode_id, content, tokenizer):
    unmask = [IGNORE_INDEX] * len(encode_id)
    ...
    try:
        ...
        return encode_id[:]  # full supervision

    leaf_strs = collect_leaf_strings(parsed)

    for leaf in leaf_strs:
        leaf_ids = tokenizer(leaf).input_ids

        if len(leaf_strs) >= 20:
            continue

        bos_token_id = tokenizer.convert_tokens_to_ids("<|startoftext|>")
        if len(leaf_ids) > 0 and leaf_ids[0] == bos_token_id:
            leaf_ids = leaf_ids[1:]

        for start in range(len(encode_id) - len(leaf_ids) + 1):
            if encode_id[start:start+len(leaf_ids)] == leaf_ids:
                for k in range(len(leaf_ids)):
                    unmask[start+k] = encode_id[start+k]
    ...
    in_value = False
    for i, tok in enumerate(encode_id):
        if tok == mdm_start_id:
            in_value = True
            unmask[i] = tok
            continue
    ...
    for i, source in enumerate(sources):
        ...
        for conv in source:
            ...
            if role in ["user", "system"]:
                ...
                # target += encode_id
                target += build_supervised_mask_for_assistant(
                    encode_id=encode_id,
                    content=content,
                    tokenizer=tokenizer
                )
...
def train(attn_implementation=None):
    ...
    # print(len(train_dataset[0]['image']))
    #
